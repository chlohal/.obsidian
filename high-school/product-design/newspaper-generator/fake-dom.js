...
FakeDomNode.prototype.appendChild = function (child) {
    if(child.constructor != FakeDomNode) throw "You cannot append a non-node!";
...
FakeDomNode.prototype.cloneNode = function () {
    if(this.value) return new FakeDomNode(this.nodeName, this.value);
    
    let copy = new FakeDomNode(this.nodeName);
    Object.assign(copy.attributes, this.attributes);
    
    for(var i = 0; i < this.childNodes.length; i++) copy.appendChild(this.childNodes[i].cloneNode());
